...
class AttentiveStatisticsPooling(nn.Module):
    ...
    def forward(self, x, weights, lengths=None):
        ...
        weights = weights.repeat(1, attn.shape[1], 1)
        # attn = attn.repeat(1, weights.shape[1], 1, 1)

        updated_attn_list = []
        for i in range(weights.shape[0]):
            if (weights[i, 0, :] != 0).sum(-1).max() > 0.1 * weights.shape[2]:
                tmp = attn[i] * weights[i].abs() / (weights[i].abs().max() + 1e-6)
                tmp = tmp.masked_fill(weights[i] == 0, float("-inf"))
                updated_attn_list.append(tmp)
            else:
                updated_attn_list.append(attn[i])
        # Reconstruct the tensor from the list
        attn = torch.stack(updated_attn_list, dim=0)
...
class ECAPA_TDNN(torch.nn.Module):
    ...
    def __init__(
        ...
        lin_neurons=192,
        activation=torch.nn.ReLU,
        channels=[1024, 1024, 1024, 1024, 3 * 1024],
        ...
        self.asp_bn = BatchNorm1d(input_size=channels[-1] * 2 // 3)

        # Final linear transformation
        self.fc = Conv1d(
            in_channels=channels[-1] * 2 // 3,
    ...
    def forward(self, x, weights, lengths=None):
        ...
        if self.add_residual:
            x = self.blocks[0](x)
            xl.append(x)
            x1 = self.blocks[1](x, lengths=lengths)
            xl.append(x1)
            x2 = self.blocks[2](x + x1, lengths=lengths)
            xl.append(x2)
            x3 = self.blocks[3](x + x1 + x2, lengths=lengths)
            xl.append(x3)
        else:
        ...
        x = self.mfa(x)
        x = rearrange(x, "b (s f) t -> (b s) f t", s=3)
        # Attentive Statistical Pooling
        x = self.asp(x, weights, lengths=lengths)
        weight_mask = weights.sum(1) > 0.1 * weights.shape[1]

        x_masked = x[weight_mask]
        x_masked = self.asp_bn(x_masked)
        x[weight_mask] = x_masked

        if not self.training:
            weight_mask = weight_mask.unsqueeze(1).unsqueeze(2)
            x = x.masked_fill(~weight_mask, float("nan"))
...
class MultiEmbedding(Model):
    ...
    def __init__(
        ...
        self.converting_linear = nn.Linear(in_features=256, out_features=128)

        self.automatic_optimization = automatic_optimization
        self.gradient_clip_val = gradient_clip_val
        self.ecapa_tdnn = ECAPA_TDNN(
            input_size=wav2vec_dim + 7, lin_neurons=embedding_dim
        )
        arc_face_loss1 = ArcFaceLoss(
            num_classes,
            192,
            margin=margin,
            scale=scale,
        ...
        self.arc_face_loss = nn.ModuleDict(
            {
                "VoxCeleb": arc_face_loss1,
                "AMI-SDM": arc_face_loss2,
            }
        )
    ...
    def forward(
        self, waveforms: torch.Tensor, y_active: torch.Tensor = None
    ) -> torch.Tensor:
        """Pass forward
        ...
        if self.hparams.lstm["monolithic"]:
            dia_outputs, (h_n, c_n) = self.lstm(dia_outputs)
        else:
        ...
        weights = (multilabel_segmentations != 0.0).transpose(2, 1)

        batch_size = emb_outputs.shape[0]
        emb_outputs = torch.cat([torch.exp(dia_outputs.detach()), emb_outputs], dim=-1)

        weights = rearrange(weights, "b s t -> (b s) t")
        emb_outputs = self.ecapa_tdnn(emb_outputs, weights)
        if emb_outputs.shape[1] == 1:
