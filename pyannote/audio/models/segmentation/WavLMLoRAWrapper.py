...
class WavLMWrapper(nn.Module):
    def __init__(self, hidden_dim=256, output_class_num=4):
    ...
    def forward(self, x, length=None):
        ...
        # # 7. Pooling
        # if length is not None:
        #     masks = torch.arange(features.size(1)).expand(
        #         length.size(0), -1
        #     ).cuda() < length.unsqueeze(1)
        #     masks = masks.float()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--adapter_hidden_dim", default=128, type=int, help="adapter dimension"
    )

    parser.add_argument(
        "--embedding_prompt_dim", default=5, type=int, help="adapter dimension"
    )
